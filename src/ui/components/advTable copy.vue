 ...
 * @version: 
 * @Author: fuhao
 * @Date: 2020-05-13 09:57:42
 * @LastEditors: xdh.ss
 * @LastEditTime: 2020-05-13 14:06:57
 -->
<template>
        ...
        <div class="adv-table_main-container">
            ...
            <div class="adv-table_header-container" ref='advTabHeaderContainer' v-if="headerSwitch" :style="{transform:`translateX(${-offset[0] || 0}%)`}">
                <table border="0" cellspacing="0" cellpadding="0" ref="tabHeader" :style="{minWidth:`${scrollConfig && scrollConfig[0] == true?'':'100%'}`}">
                    <tr>
                        <th v-if="hcell.toggle ? hcell.toggle($route, outParams) : true" v-for="(hcell,i) in columns" :key="i"  :style="thCellStyle(hcell)">
                            <div class="adv-header-col">
            ...
            <div class="adv-table_search-container" ref='advTabSearchContainer' v-if="searchSwitch">
                <table border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        <th v-if="hcell.toggle?hcell.toggle($route, outParams):true" v-for="(hcell,i) in columns" :key="i" :style="thCellStyle(hcell)">
                            <div class="adv-header-col" v-if="hcell.search">
                                            ...
                                            <span v-if="col.type == 'simple'" class="cell-simple" :style="col.wordStyle ? col.wordStyle(row, outParams):{}" >
                                                <template v-if="col.simpleContent">
                                                    <div style="display:flex;flex-flow:row;align-items: center;justify-content: space-between;padding:0 10px;">
                                                        <span>
                                                        <label v-for="(lab, labi) in col.simpleContent(row, col, ri)">
                                                            <label :style="lab.flag?{color:'red'}:{}">{{lab.txt}}</label><label v-if="!lab.end">,</label>
                                                        </label>&emsp;
                                                        </span>
                                                        <!--<el-select v-model="row[col.extendKey]"  v-if="syncSimpleContentIcon(row, col, ri)" size="mini">-->
                                                            <!--<el-option label="一类" value=""></el-option>-->
                                                            <!--<el-option label="二类" value=""></el-option>-->
                                                        <!--</el-select>-->
...
<!--                                                        @input="handleEditCellInput(row, ri, col.key)"-->
                                                    ...
                                                    <template v-if="col.editConfig && col.editConfig.type == 'textarea'">
                                                        <el-input type="textarea" autosize v-if="col.editConfig.switch(row, params, ri, ci, handlePoint)"
                                                                  ...
                                                                  :style="inputStyleExtend(row, col, ri, ci)"
                                                        />
<!--                                                        @keydown.enter.stop="handleEditCellInput(row, ri, col.key)"-->
                                                        <!--                                                        @input="handleEditCellInput(row, ri, col.key)"-->
                                                        <template v-else>
                                                            <label style="text-overflow: ellipsis;white-space:normal;" class="cell-simple"> {{ syncDisplay(row, col, ri)}} </label>
                                                        </template>
                                                    ...
                                                    <template v-if="col.editConfig && col.editConfig.type == 'scooterCode'">
                                                        <template  v-if="col.editConfig.switch(row, params, ri, ci, handlePoint)">
                                                            <el-select
                                                                    ...
                                                                    reserve-keyword
                                                                    placeholder="请输入板车号"
                                                                    @change=" id => handleInsertScooterChange(id, row, ri, col.key)"
                                                                    :remote-method="handleSearchScooters"
                                                                    style="width:12rem;"
                                                            >
                                                                <el-option
                                                                        v-for="item in outParams.scooterOptions"
                                                                        :key="item.id"
                                                                        :label="item.scooterCode"
                                                                        :value="item.id"
                                                                >
                                                                </el-option>
                                                            </el-select>
                                                    ...
                                                    <template v-if="col.editConfig && col.editConfig.type == 'uld'">
                                                        <template  v-if="col.editConfig.switch(row, params, ri, ci, handlePoint)">
                                                            <template v-if="row.scooterType != 9">
                                                                滚筒编号<input type="text" v-model="row.scooterCode" style="width:10%;background-color:#fff" placeholder="编号"/>
                                                                滚筒重量<input type="text" v-model="row.scooterWeight" style="width:8%;background-color:#fff" placeholder="类型"/>
                                                            </template>
                                                            <template v-if="row.scooterType == 9">
                                                                类型<label style="width:10%;text-align:center;font-weight:800;">CAR</label>
                                                                编号<input type="text" v-model="row.scooterCode" @input="handleULDModelChange(row, 'code')" style="width:15%;" placeholder="编号"/>
                                                                航司<label style="width:10%;text-align:center;font-weight:800;">JC</label>
                                                                自重<input type="text" v-model="row.scooterWeight" @input="handleULDModelChange(row, 'weight')" style="width:10%;" placeholder="自重"/>
                                                            </template>
                                                            <template v-else>
                                                                类型<input type="text" v-model="row.uldType" class="uppercase" @input="handleULDModelChange(row, 'type')" style="width:6%;" placeholder="类型"/>
                                                                编号<input type="text" v-model="row.uldCode" @input="handleULDModelChange(row, 'code')" style="width:10%;" placeholder="编号"/>
                                                                航司<input type="text" v-model="row.iata" class="uppercase" @input="handleULDModelChange(row, 'iata')" style="width:6%;" placeholder="航司"/>
                                                                自重<input type="text" v-model="row.uldWeight" @input="handleULDModelChange(row, 'weight')" style="width:8%;" placeholder="自重"/>
                                                            </template>
                                                            <i class="fa fa-check" style="margin:0 .5rem;cursor:pointer;" @click.stop="uldSwitchChange(row, col, false)"></i>
                                                        </template>
                                                        <template v-else>
                                                            <label style="text-overflow: ellipsis;white-space:normal;" class="cell-simple"> {{ syncDisplay(row, col, ri)}} </label>
                                                            <i v-if="col.editConfig.edit(row)" class="fa fa-edit" style="margin:0 .5rem;cursor:pointer;" @click.stop="uldSwitchChange(row, col, true)"></i>
                                                        </template>
                                                    </template>
                            ...
                            <td v-if="hcell.edit && hcell.toggle?hcell.toggle($route, outParams):true" :style="tdColStyle(hcell, editRowBind)">
                                <div class="adv-header-col p-cell" v-if="hcell.edit" :data-px="hi" :data-py="-1" :style="editPointStyle(hi, -1)" :class="editPointClass(hi, -1)"  @click="handlePointSet(editRowBind, hcell, hi, -1)">
                                    ...
                                    <template v-if="hcell.edit.type == 'operate'" >
                                        <div v-if="operate.switch?operate.switch(editRowBind, params):true" v-for="(operate, ind) in hcell.edit.operates" class="operate-cell" >
                                            <el-button
            ...
            <div class="adv-table_total-container" ref='advTabTotalContainer' v-if="totalLine">
                <table border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        <td v-if="hcell.toggle?hcell.toggle($route, outParams):true" v-for="hcell in columns" :style="tdTotalColStyle(hcell)">
                            <!--            <td v-if="hcell.toggle?hcell.toggle($route):true" v-for="hcell in columns">-->
                            <div class="adv-header-col" v-if="hcell.total">
        ...
        <!--        <Dialog title="运单信息" :show="waybillDetailSwitch" @close="waybillDetailSwitch = false">-->
        <!--            <div class="waybill-detail-dialog">-->
        <!--                <WaybillDetail :waybillId="waybillDetailId" :config="outParams?outParams.waybillDetailConfig:{}"></WaybillDetail>-->
        <!--            </div>-->
        <!--        </Dialog>-->
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleULDModelChange(row) {
                /*if(row.scooterType == 9) {*/
                    /*extend(row, {*/
                        /*scooterCode:this.uldModel.code,*/
                        /*iata:'JC',*/
                        /*scooterWeight:this.uldModel.weight,*/
                    /*})*/
                /*}else {*/
                    /*extend(row, {*/
                        /*uldType: this.uldModel.type,*/
                        /*uldCode:this.uldModel.code,*/
                        /*iata:this.uldModel.iata,*/
                        /*uldWeight:this.uldModel.weight,*/
                    /*})*/
                /*}*/
            },
            uldSwitchChange(row, col, sw) {
                /*if(row.scooterType == 9) {*/
                    /*extend(this.uldModel, {*/
                        /*type: 'CAR',*/
                        /*code: row.scooterCode,*/
                        /*iata: 'JC',*/
                        /*weight: row.scooterWeight,*/
                    /*});*/
                /*}else {*/
                    /*extend(this.uldModel, {*/
                        /*type: row.uldType,*/
                        /*code: row.uldCode,*/
                        /*iata: row.iata,*/
                        /*weight: row.uldWeight,*/
                    /*});*/
                /*}*/

                if(!sw) {
                    this.$emit('handleEditCellInput', { row })
                }

                this.$set(row, 'uldSwitch', sw);
            },
            editDefaultCheck() {
                if(this.editLine) {
        ...
        computed: {
            ...
            thCellStyle(c) {
                return function(c) {
                    ...
                    w = w?Number(w.slice(0, -2))/12 + 'rem': '';

                    let left = get(c, 'left', '.4rem');
                    let right = get(c, 'right', '.4rem');
                    return extend({
                        textAlign: get(c, 'align', 'center'),
                        width: w,
                        maxWidth: w,
                        minWidth: w,
                        padding: `.8rem ${right} .8rem ${left}`
            ...
            tdTotalColStyle(c, row) {
                return function(c, row) {
                    ...
                    let left = get(c, 'left', '.2rem');
                    let right = get(c, 'right', '.2rem');
                    return extend({
                        textAlign: get(c, 'align', 'center'),
                        width: w,
                        maxWidth: w,
                        minWidth: w,
                        padding: `.2rem ${right} .2rem ${left}`
            ...
            rowStyleExtend(row, ri, e) {
                return (row, ri, e) => {
                    ...
                    // let ele = this.$refs[`row_${ri}`];
                    // let parent = this.$refs['tab_cur'];
                    // if(ele && ele[0] && parent) {
                    //     let rc = ele[0].getBoundingClientRect();
                    //     let rp = parent.getBoundingClientRect();
                    //
                    //     let floor = rp.y + rp.height;
                    //
                    //     this.log.info('row visibility：', floor, rc.y, rc, rc.y > floor);
                    //     if(rc.y > floor) {
                    //         // extend(styleExtend, { visibility:'hidden' })
                    //         extend(styleExtend, { color:'red' })
                    //     }else {
                    //         // extend(styleExtend, { visibility:'visible' })
                    //         extend(styleExtend, { color:'green' })
                    //     }
                    // }
                    return styleExtend;
    ...
    .adv-table-container {
        ...
        .adv-table_foot-container {
            ...
            .tab-pagination {
                ...
                box-shadow: 0 0.07143rem 0.35714rem #bbb;
                border: solid 0.07143rem #ababab;
                .select-num{
                    display: inline-block;
